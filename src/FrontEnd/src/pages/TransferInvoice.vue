<template>
  <div class="transferInvoice">
    <div>
      <span class="heading">
        转让单据
      </span>
    </div>
    <div class="transferInvoiceForm">
      <a-form
        :form="form"
        layout="horizontal"
        @submit="handleSubmit"
      ...
      >
        ...
        <a-form-item label="金额" :validate-status="passwordError() ? 'error' : ''" :help="passwordError() || ''">
          <a-input
            v-decorator="[
              'invoiceAmount',
        ...
        if (!err) {
          ...
          this.axios.request({
            ...
            if (response.status == 200) {
              ...
              that.addTfInvoice(response.data.id)
